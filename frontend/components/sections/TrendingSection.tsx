...
export default function TrendingSection() {
  ...
  // Filter posts where category = "TRENDY WEAR" (case-insensitive), sorted by date descending, max 3
  let trendyPosts = getPostsByCategory(posts, "TRENDY WEAR", 3);

  // Graceful fallback: If no trendy wear posts found, use top 3 newest posts
  if (trendyPosts.length === 0) {
    trendyPosts = sortPostsByDateDesc(posts).slice(0, 3);
  }
    ...
    <section className="bg-surface-container-low border border-primary/5 px-[20px] md:px-[80px] py-[120px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-[32px]">
        {trendyPosts.map((item) => (
          ...
          >
            <Link href={`/posts/${item.slug}`} className="flex flex-col h-full">
